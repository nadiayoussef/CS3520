...
template <class T>
CommaSeparatedList<T>::CommaSeparatedList(T *d, int s) // missing * as d was defined as a pointer variable in the class
{
   data = d;
   size = s;
}
...
template <class T>
    void CommaSeparatedList<T>::printList() // flip the second < to > so that the T is enclosed within carrot brackets
{
   cout << "Comma separated list:" << endl; // flip >> to << after cout
   for (int x = 0; x < size; ++x)
   {
      cout << data[x]; // replace y with x as x is the defined variable in this scope
      if (x != size - 1)
         cout << ", ";
   }
   cout << endl << endl;
...
ostream &operator<<(ostream &out, const Book &b)
{
   ...
   return out; // change return from ostream (which is just the return type) to out
}
...
int main()
{
   ...
   CommaSeparatedListSize = sizeof(someBook) / sizeof(someBook[0]); // typo - sizeif should be sizeof with an "o"
   ...
   CommaSeparatedListOfBooks.printList(); // typo - prinList should be printList with a "t"
